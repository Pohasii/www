...
if ($result[0]['special']!=1) {
	...
	if($result[0]['special']==1) {
		...
		if($_POST['sub'] == 'Обновить'){
			...
			$res = put("UPDATE `tournaments` SET `title`='$title',`demotitle`='$demotitle',`format`='$format',`fulltext`='$fulltext',`specification`='$specification',`date`='$date',`status`='$status' WHERE `id`='$id'");
		...
		if($_POST['sub'] == 'Создать турнир'){
			...
			$res = put("INSERT INTO `tournaments`(`title`, `demotitle`, `format`, `fulltext`, `specification`,`date`, `status`) VALUES ('$title', '$demotitle', '$format', '$fulltext', '$specification', '$date', '$status')");
